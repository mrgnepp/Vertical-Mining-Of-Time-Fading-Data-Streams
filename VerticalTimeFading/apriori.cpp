...
void apriori::do_apriori(std::string filename, double minsup, double fade_factor, int chunkSize) {
	...
	std::vector<std::vector<transaction>> batches = apriori::split_into_batches(transactions, chunkSize);
	...
	for (auto batch : batches) {
		...
		auto candidate_itemsets = apriori::generate_candidate_itemsets(freq_singletons, level);
		...
		while (!candidate_itemsets.empty()) {
			...
			// generate
			candidate_itemsets = apriori::generate_candidate_itemsets(level_freq_itemsets, level);
